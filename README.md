# Sahiel Bose's Website

Sahiel Bose's Website
A personal website built and deployed by Sahiel Bose.
Live site: sahiel-bose.vercel.app

About
This is the source code for my personal website. Feel free to explore the code, and if you'd like to suggest changes or improvements, see the contributing section below.

Getting Started
Prerequisites
Make sure you have the following installed:

Node.js (v18 or higher recommended)
Git

Installation

Clone the repository

bash   git clone https://github.com/sahielbose/Sahiel-Bose-Website.git
   cd Sahiel-Bose-Website

Install dependencies

bash   npm install

Run the development server

bash   npm run dev

Open http://localhost:3000 in your browser to view it locally.


Project Structure
Sahiel-Bose-Website/
├── SahielBose_Website/     # Main website source files
│   ├── ...
├── README.md

Making Changes
Editing the Code

Fork the repository by clicking the Fork button at the top right of the GitHub page
Clone your fork locally:

bash   git clone https://github.com/YOUR-USERNAME/Sahiel-Bose-Website.git

Create a new branch for your changes:

bash   git checkout -b your-feature-branch

Make your edits in the SahielBose_Website/ folder
Commit and push your changes:

bash   git add .
   git commit -m "Describe your changes here"
   git push origin your-feature-branch

Open a Pull Request on GitHub so changes can be reviewed

Suggesting Changes Without Coding
If you'd like to suggest a change but don't want to edit the code yourself, open an Issue and describe what you'd like to see!

Deployment
This site is deployed on Vercel. Any changes pushed to the main branch are automatically deployed to the live site.

Contact
Have questions or want to get in touch? Visit the live site at sahiel-bose.vercel.app for contact info.
