  ...
  experience: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Designed and implemented a machine learning model to optimize data processing and analysis, reducing noise and improving detection accuracy, while building the React Native companion app that delivers live overdose alerts to patients and first responders, and co-authoring an academic paper and literature review on opioid overdose trends and detection methodologies (patent and copyright in progress).",
      ],
    ...
    },
    {
      ...
      bullets: [
        ...
        "Set up and integrated the OpenAI API to run large language model experiments, building and querying Prolog knowledge bases using SWI-Prolog and pyswip, and developing pipelines to translate natural language queries into symbolic logic and back into readable output, while supporting ProSLM, the lab's symbolic-logic-augmented small language model platform, through the hosting and integration layer that connects its research components into one deployable system.",
    ...
    },
    {
      ...
      bullets: [
        ...
        "Analyzed advanced GNN architectures including HiGPT, HMSG, and GRAPES, assessing their applicability to heterogeneous road network datasets and contributing to model development and optimization strategies for surfacing congestion hotspots from large-scale, real-time geospatial road-network data.",
      ],
    ...
    <aside className="sidenav">
      <a href="#intro" className="logo sidenav-logo" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
        <span className="logo-mark">
          <svg viewBox="0 0 32 32" width="32" height="32">
            <circle cx="16" cy="16" r="15.2" fill="#0e0e14" stroke="currentColor" strokeWidth="1.1"/>
            <text x="16" y="22" textAnchor="middle" fontFamily="var(--serif)" fontSize="18" fontStyle="italic" fill="currentColor" letterSpacing="-0.5">SB</text>
          </svg>
        </span>
        <span className="logo-text">
          <span className="logo-name">Sahiel Bose</span>
      ...
      <div className="sidenav-progress" aria-hidden="true"><span style={{ clipPath: `inset(0 0 ${(100 - progress * 100).toFixed(2)}% 0)` }} /></div>
    </aside>
  );
}
    ...
    <div ref={ref} className={`xp-entry ${inView ? 'in' : ''}`}>
      ...
      <article className="xp-card">
        <div className="xp-panel-head">
          <div>
            <div className="mono tag">0{i + 1} · {x.kind} · {x.location}</div>
