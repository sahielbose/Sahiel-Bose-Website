...
const DATA = {
  ...
  experience: [
    {
      ...
      bullets: [
        "Lead the design and development of Optivia's AI agents as primary architect of its core intelligence, agentic workflows, and decision-making logic, turning a natural-language prompt into a coordinated, dependency-aware fleet of specialized coding sub-agents (patent pending).",
        "Designed the math and logic behind the pipeline: prompt routing, a five-signal complexity scoring formula, critical-path scheduling, model selection, and a per-agent reflection layer that keeps hallucinations from spreading between agents.",
        "Built a framework that intercepts user inputs, enriches prompts with targeted clarifying questions, and drives Claude Code through the Anthropic SDK, while serving as lead author on the research paper documenting the system.",
      ],
    ...
    },
    {
      ...
      bullets: [
        "Built an OpenSwarm library with MCP server connectivity and a swappable database layer supporting two CRM integrations, Admin and User, letting agents read and write structured data across external services directly from the OpenSwarm environment.",
        "Redesigned the OpenSwarm landing page from a blank slate into a dimensional, interactive site with custom anime.js animations, staggered agent-node physics, SVG motion-path connections, and a scroll-driven hero.",
        "Designed and built a full-stack LinkedIn-gated access flow on FastAPI and React, engineering a five-outcome state machine with comment verification, one-time-use redemption, profile URL normalization, and an authenticated admin panel.",
      ],
      stack: [],
    },
    {
      ...
      bullets: [
        "Contributing to the R&D of a patent-pending opioid overdose detection device that monitors continuous physiological signals to catch overdose events in real time, engineering features that capture the overdose signature and running risk analysis to guide hardware-software integration.",
        "Built and benchmarked three ML classifiers (logistic regression, LSTM, and GRU) on a large physiological dataset, selecting the GRU for fast on-device, real-time detection.",
        "Improved data processing to cut noise and boost detection accuracy, built the React Native companion app that delivers live overdose alerts to patients and first responders, and co-authored an academic paper and literature review on overdose detection (patent and copyright in progress).",
      ],
    ...
    },
    {
      ...
      bullets: [
        "Completed the AIEA Lab's undergraduate research program, working through a structured series of LLM and logic research tasks covering symbolic reasoning, logical inference, and AI explainability across Python and Prolog; now collaborating on academic research into logical reasoning in LLMs and methods to improve their problem-solving.",
        "Built pipelines that translate natural-language queries into symbolic logic and back into readable output, querying Prolog knowledge bases through SWI-Prolog and the OpenAI API, and support ProSLM, the lab's logic-augmented language model platform.",
        "Researched frameworks like LOGIC-LM and deontic logic and their applications in legal AI, agent constraints, and AI ethics, and implemented a backward-chaining inference engine for first-order logic from scratch in Python.",
      ],
    ...
    },
    {
      ...
      bullets: [
        "Assist a Ph.D. researcher on a GNN-based project detecting traffic congestion hotspots across large-scale road networks, applying AI and spatial analytics to heterogeneous, real-time geospatial datasets.",
        "Sourced and evaluated 15+ papers on hotspot detection and graph machine learning across multiple research cycles, then trained and benchmarked sampling methods like GraphSAGE, GraphSAINT, and ShadowGraph on large road-network graphs.",
        "Analyzed advanced GNN architectures including HiGPT, HMSG, and GRAPES, assessing their fit for heterogeneous road networks and contributing to model development and optimization for congestion prediction.",
      ],
  ...
  projects: [
    ...
    },
    {
      ...
      depth: [
        "Converts a voice transcript into FHIR R4 observations and ICD-10 codes, runs differential diagnosis with confidence scoring, and screens orders for contraindications (catches Warfarin + tPA at 100% in evals).",
      ...
      tags: ["Multi-Agent", "FHIR R4", "Healthcare"],
    ...
    },
    {
      ...
      depth: [
        "A fine-tuned MobileNetV2 CNN hits 95% validation accuracy on 36 static signs; a bidirectional LSTM and TCN reach 90% on dynamic gestures after augmenting under 500 clips into 14,000+ samples.",
  ...
  skills: {
    ...
    "Frameworks & Libraries": ["PyTorch", "TensorFlow", "Keras", "LangChain", "FastAPI", "OpenCV", "MediaPipe", "ONNX", "React Native", "Next.js", "MCP"],
        ...
        <a href="#intro" className="logo" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
          <span className="logo-mark">
            <svg viewBox="0 0 64 64" width="32" height="32">
              <rect x="2" y="2" width="60" height="60" rx="16" fill="oklch(0.16 0.04 262)" stroke="oklch(0.68 0.16 260 / 0.5)" strokeWidth="1.5"></rect>
  ...
  return (
    <section id="experience" className="section">
      ...
      <div className="xp-flow" ref={flowRef}>
        <div className="xp-rail" aria-hidden="true"><span className="xp-rail-fill" /></div>
...
function XpEntry({ x, i }) {
  ...
  return (
    <div ref={ref} className={`xp-entry ${inView ? 'in' : ''}`}>
      <div className="xp-node" aria-hidden="true">
        <span className="xp-node-dot" />
      </div>
      <article className="xp-card">
        <div className="xp-panel-head">
          <div>
            <div className="mono tag">0{i + 1} · {x.location}</div>
    ...
    <article ref={ref} className={`proj-row ${i % 2 ? 'rev' : ''} ${inView ? 'in' : ''}`}>
      <div className="proj-row-body">
        <div className="proj-row-meta mono">
          <span className="proj-row-period">{p.period}</span>
          <span className="proj-row-dot">·</span>
          <span className="proj-row-status">{p.status}</span>
        </div>
        <h3 className="proj-row-title">{p.title}</h3>
        <div className="proj-row-sub mono">{p.subtitle}</div>
